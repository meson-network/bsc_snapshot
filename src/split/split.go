...
func Split(originFilePath string, destDir string, sizeStr string, thread int) error {
...
func splitFile(originFilePath string, destDir string, chunkSize int64, thread int) error {
	...
	// json file
	splitConfig := model.FileConfig{
		RawFile: model.RawFileInfo{
			FileName: fileInfo.Name(),
			Size:     fileInfo.Size(),
		},
		EndPoint:        []string{},
		ChunkedFileList: []model.ChunkedFileInfo{},
	}
	...
	for i := int64(1); i <= int64(num); i++ {
		...
		go func() {
			...
			splitConfig.ChunkedFileList = append(splitConfig.ChunkedFileList, model.ChunkedFileInfo{
				FileName: name,
				Md5:      md5Str,
				Size:     fileSize,
	...
	configFilePath := filepath.Join(destDir, model.DEFAULT_CONFIG_NAME)
